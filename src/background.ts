...
function persistLogsSoon() {
  try {
    if (persistLogsTimer) {
      clearTimeout(persistLogsTimer as any);
    }
  } catch {}
...
async function updateBadge() {
  ...
  try {
    await act.setIcon({ path: working ? ICONS.yellow : ICONS.blue });
  } catch {}
}
...
function ensureEnqueued(regDomain: string, origHost: string, source: string) {
  ...
  if (domainStatus[regDomain]) {
    if (
      domainStatus[regDomain].status !== 'pending' &&
      domainStatus[regDomain].status !== 'error'
    ) {
